...
exportCDS <- function(monocle_cds, export_to = c("Seurat", "Scater"), export_all = FALSE) {
  if (export_to == "Seurat") {
    ...
    export_cds <- Seurat::CreateSeuratObject(
      raw.data = data,
    ...
    if (!exists("newSCESet", where = asNamespace("scater"), mode = "function")) {
      stop(
        "scater::newSCESet is no longer available in newer versions of scater. ",
        "Please use an older version of scater or convert to a different format."
      )
    }
    ...
    export_cds <- scater::newSCESet(
      exprsData = data, countData = NULL, tpmData = NULL,
...
importCDS <- function(otherCDS, import_all = FALSE) {
  ...
  } else if (inherits(otherCDS, "SCESet")) {
    requireNamespace("scater")
