...
#' @importFrom igraph graph_from_adjacency_matrix V dfs get.all.shortest.paths
...
#' @importFrom igraph V vertex degree get_diameter edge graph.neighborhood
...
pq_helper <- function(mst, use_weights = TRUE, root_node = NULL) {
  ...
  if (is.null(root_node) == FALSE) {
    ...
    if (use_weights) {
      diam <- V(mst)[get_diameter(mst)]
    } else {
      diam <- V(mst)[get_diameter(mst, weights = NA)]
    }
  }
...
make_canonical <- function(pq_tree) {
  ...
  {
    child_of_p_node <- V(canonical_pq)[suppressWarnings(igraph::.nei(p_node, mode = "out"))]
    parent_of_p_node <- V(canonical_pq)[suppressWarnings(igraph::.nei(p_node, mode = "in"))]
...
count_leaf_descendents <- function(pq_tree, curr_node, children_counts) {
  if (V(pq_tree)[curr_node]$type == "leaf") {
    ...
    children_count <- 0
    for (child in V(pq_tree)[suppressWarnings(igraph::.nei(curr_node, mode = "out"))])
    {
...
extract_good_branched_ordering <- function(orig_pq_tree, curr_node, dist_matrix, num_branches, reverse_main_path = FALSE) {
  ...
  for (i in 1:num_branches)
  {
    ...
    parents <- V(pq_tree)[suppressWarnings(igraph::.nei(names(branch_node_counts)[i], mode = "in"))]
    if (length(parents) > 0 && parents$type == "P") {
      p_node_parent <- V(pq_tree)[suppressWarnings(igraph::.nei(names(branch_node_counts)[i], mode = "in"))]
      parent_branch_id <- V(pq_tree)[suppressWarnings(igraph::.nei(p_node_parent, mode = "in"))]$name
      branch_tree <- branch_tree + edge(parent_branch_id, branch_id)
    }
    pq_tree[V(pq_tree)[suppressWarnings(igraph::.nei(names(branch_node_counts)[i], mode = "in"))], names(branch_node_counts)[i]] <- FALSE
  }
  ...
  extract_branched_ordering_helper <- function(branch_tree, curr_branch, cell_ordering_tree, branch_pseudotimes, dist_matrix, reverse_ordering = FALSE) {
    ...
    for (child in V(branch_tree)[suppressWarnings(igraph::.nei(curr_branch, mode = "out"))])
    {
  ...
  assign_cell_state_helper <- function(ordering_tree_res, curr_cell) {
    ...
    children <- V(cell_tree)[suppressWarnings(igraph::.nei(curr_cell, mode = "out"))]
  ...
  assign_pseudotime_helper <- function(ordering_tree_res, dist_matrix, last_pseudotime, curr_cell) {
    ...
    V(cell_tree)[curr_cell]$parent <- V(cell_tree)[suppressWarnings(igraph::.nei(curr_cell, mode = "in"))]$name
...
#' @importFrom igraph V mst graph_from_adjacency_matrix degree get_diameter dfs
...
select_root_cell <- function(cds, root_state = NULL, reverse = FALSE) {
  if (is.null(root_state) == FALSE) {
    ...
    diameter <- get_diameter(minSpanningTree(cds))
...
#'
#' @importFrom stats dist
#' @importFrom igraph graph_from_adjacency_matrix V as.undirected
#'
...
project2MST <- function(cds, Projection_Method) {
  ...
  if (!is.function(Projection_Method)) {
    ...
    for (i in 1:length(closest_vertex)) {
      neighbors <- names(V(dp_mst)[suppressWarnings(igraph::.nei(closest_vertex_names[i], mode = "all"))])
      projection <- NULL
      distance <- NULL
      Z_i <- Z[, i]
