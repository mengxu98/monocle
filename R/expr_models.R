...
fit_model_helper <- function(
  ...
  if (any(expressionFamily@vfamily %in% c("negbinomial", "negbinomial.size"))) {
    ...
    if (is.null(disp_func) == FALSE) {
      disp_guess <- calculate_NB_dispersion_hint(
        disp_func,
        round(orig_x)
      )
...
fitModel <- function(cds,
                     ...
                     cores = 1) {
  if (cores > 1) {
    f <- mcesApply(cds, 1, fit_model_helper,
      required_packages = c("BiocGenerics", "Biobase", "VGAM", "plyr", "Matrix"), cores = cores,
