...
opt_norm_t <- function(t, fpkm, mRNAs_for_mode, kb_slope, kb_intercept, expr_thresh = 0.1, pseudocnt = NULL, return_norm = FALSE) {
  ...
  if (!is.null(pseudocnt)) {
    10^dmode(log10(selected + pseudocnt))
  } else {
...
estimate_t <- function(relative_expr_matrix, relative_expr_thresh = 0.1) {
  unlist(apply(relative_expr_matrix, 2, function(relative_expr) 10^mean(dmode(log10(relative_expr[relative_expr > relative_expr_thresh])))))
}
...
relative2abs <- function(
  ...
  if (!is.null(ERCC_controls) | !is.null(ERCC_annotation)) {
    ...
    norm_fpkms <- mcmapply(function(cell_exprs, molModel) {
      ...
      each = nrow(relative_expr_matrix)
    )), molModels, mc.cores = cores)
    ...
    if (return_all == T) {
      return(list(
        norm_cds = norm_fpkms, kb_slope = kb_slope, kb_intercept = kb_intercept,
    ...
    if (return_all == T) {
      return(list(
        norm_cds = census_transcript_counts,
