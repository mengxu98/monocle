...
HSMM <- reduceDimension(HSMM,
  max_components = 2, num_dim = 3, norm_method = "log", reduction_method = "tSNE",
...
test_that(
  "differentialGeneTest functions under standard conditions (conditions set in vignette)",
  expect_error(differentialGeneTest(HSMM_myo[expressed_genes, ], fullModelFormulaStr = "~Media"), NA)
)

test_that(
  "Error is thrown if cds is not of type 'CellDataSet'",
...
test_that(
  "Error is thrown if i) relative_expr is set to true, ii) expression family is 'negbinomial' or 'negbinomial.size', iii) size factors of cds is null",
...
test_that(
  "Error is thrown if i) relative_expr is set to true, ii) expression family is 'negbinomial' or 'negbinomial.size', iii) sum of size factors of cds is NA",
  expect_error(differentialGeneTest(HSMM_myo, fullModelFormulaStr = "~Media"), "Error: to call this function with relative_expr==TRUE, you must first call estimateSizeFactors() on the CellDataSet.")
)
