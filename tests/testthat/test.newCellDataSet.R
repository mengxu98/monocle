...
test_that("newCellDataSet is able to create a cell data set with only phenoData inputted from user", {
  expect_message(newCellDataSet(as.matrix(HSMM_expr_matrix), phenoData = pd), "featureData must contain a column verbatim named 'gene_short_name' for certain functions")
})
...
test_that("newCellDataSet throws warning if user inputs a featureData table that doesn't contain a column named 'gene_short_name'", {
  ...
  fd <- new("AnnotatedDataFrame", data = HSMM_gene_annotation)
  expect_message(newCellDataSet(as.matrix(HSMM_expr_matrix), featureData = fd), "featureData must contain a column verbatim named 'gene_short_name' for certain functions")
})
