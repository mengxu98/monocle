...
// Project cells (columns of z) onto the segments of the minimum spanning tree
// over the DDRTree centers (columns of y), and build the minimum spanning tree
// over the projected cells with Prim's algorithm. Neighbor iteration follows
// ascending center order so tie-breaking matches igraph::adjacent_vertices.
// [[Rcpp::export]]
List ddrtree_project_cells_to_mst_cpp(
    const NumericMatrix& z,
    const NumericMatrix& y,
    const IntegerMatrix& graph_edges,
    const IntegerVector& closest_vertex) {
  const int dims = z.nrow();
  const int n_cells = z.ncol();
  const int n_centers = y.ncol();
  if (y.nrow() != dims) {
    Rcpp::stop("z and y must have the same number of rows");
  }
  if (closest_vertex.size() != n_cells) {
    Rcpp::stop("closest_vertex length must equal the number of cells");
  }
  if (graph_edges.ncol() != 2) {
    Rcpp::stop("graph_edges must have two columns");
  }
...
// Traverse a tree from a root cell with a preorder DFS over ascending neighbor
// order, so the visit order and parent assignment match igraph::dfs() on the
// same tree. Pseudotime accumulates the given per-edge weights; a new state
// starts whenever the parent vertex is a branch node (degree > 2).
// [[Rcpp::export]]
List ddrtree_order_from_edges_cpp(
    int n_vertices,
    const IntegerMatrix& edges,
    const NumericVector& weights,
    int root_cell = 1) {
  if (n_vertices < 1) {
    Rcpp::stop("n_vertices must be positive");
  }
  if (edges.ncol() != 2) {
    Rcpp::stop("edges must have two columns");
  }
  if (weights.size() != edges.nrow()) {
    Rcpp::stop("weights length must match edge rows");
  }

  std::vector<std::vector<std::pair<int, double>>> adj(n_vertices);
  for (int i = 0; i < edges.nrow(); ++i) {
    const int from = edges(i, 0) - 1;
    const int to = edges(i, 1) - 1;
    if (from < 0 || from >= n_vertices || to < 0 || to >= n_vertices || from == to) {
      continue;
    }
  ...
  if (root < 0 || root >= n_vertices) root = 0;

  std::vector<int> parent(n_vertices, -1);
  std::vector<double> parent_weight(n_vertices, 0.0);
  ...
  NumericVector pseudotime(n_vertices, 0.0);
  IntegerVector state(n_vertices, 1);
  IntegerVector parent_out(n_vertices, 0);
  int curr_state = 1;
...
// Select the graph vertex (closest DDRTree center) that should root the
// trajectory when re-rooting by state: build a minimum spanning tree over the
// candidate cells, walk its diameter, and pick the endpoint candidate with the
// smallest (or largest) pseudotime, mirroring select_root_cell() in R.
// [[Rcpp::export]]
int ddrtree_select_root_by_state_cpp(
    const NumericMatrix& coords,
    const IntegerVector& candidate_cells,
  ...
  if (n_candidates < 1) {
    Rcpp::stop("candidate_cells must not be empty");
  }
  if (pseudotime.size() != n_total) {
    Rcpp::stop("pseudotime length must match coords columns");
  }
  if (closest_vertex.size() != n_total) {
    Rcpp::stop("closest_vertex length must match coords columns");
  }

  if (n_candidates == 1) {
    const int cell = candidate_cells[0] - 1;
    if (cell < 0 || cell >= n_total) {
      Rcpp::stop("candidate_cells contains an invalid index");
    }
